...
def main():
    ...
    # Resolve active memory directory to lock
    ...
    if not os.path.exists(ISA_MEMORY_DIR):
        try:
            os.makedirs(ISA_MEMORY_DIR)
        except Exception:
            pass

    lock_dir = os.path.join(ISA_MEMORY_DIR, "kazumi.lock")
    with FolderLock(lock_dir):
        try:
            bot = Kazumi()
